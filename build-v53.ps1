...
try {
    ...
    $root = "$substDrive\"
    $out = Join-Path $root 'build\v141-expert-raw-sdhms-v36-webdav-audio-eraser-native-diag-7.4.12.3'
    ...
    $unsignedApk = Join-Path $out 'SamsungFeatureExtension-v1.3.5-ExpertRAW-SDHMS-WebDAV-AudioEraser-NativeDiag-unsigned.apk'
    Invoke-Checked -FilePath $java -Arguments @(
    ...
    $finalApk = Join-Path $out 'SamsungFeatureExtension-v1.3.5-ExpertRAW-SDHMS-WebDAV-AudioEraser-NativeDiag.apk'
    ...
    $rootOutput = Join-Path $root 'SamsungFeatureExtension-v1.3.5-ExpertRAW-SDHMS-WebDAV-AudioEraser-NativeDiag.apk'
