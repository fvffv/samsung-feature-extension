param(
    [int]$BuildCode = 142,
    [string]$OutputVersion = '1.3.6',
    [string]$OutputTag = 'ExpertRAW-SDHMS-WebDAV-AudioEraser-JniCwd',
    [string]$BuildFlavor = 'expert-raw-sdhms-v36-webdav-audio-eraser-jni-cwd-7.4.12.3'
)
...
try {
    ...
    $root = "$substDrive\"
    $out = Join-Path $root "build\v$BuildCode-$BuildFlavor"
    ...
    Invoke-Checked -FilePath $java -Arguments @(
        '-jar', $apktool, 'b', '-a', $aapt2.FullName, $apkProject, '-o', $resourceApk
    )
    ...
    $unsignedApk = Join-Path $out "SamsungFeatureExtension-v$OutputVersion-$OutputTag-unsigned.apk"
    Invoke-Checked -FilePath $java -Arguments @(
        '-jar', $apktool, 'b', '-a', $aapt2.FullName, $apkProject, '-o', $unsignedApk
    )
    ...
    $finalApk = Join-Path $out "SamsungFeatureExtension-v$OutputVersion-$OutputTag.apk"
    ...
    $rootOutput = Join-Path $root "SamsungFeatureExtension-v$OutputVersion-$OutputTag.apk"
