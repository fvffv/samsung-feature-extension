...
$root = $scriptDir
$out = Join-Path $root 'build/v83-oneui-font-file-style-menu'
...
& "$bt/apksigner.bat" sign `
    ...
    --out "$out/SamsungFeatureExt-v2.9.18.apk" `
    "$out/aligned.apk"
& "$bt/apksigner.bat" verify --verbose --print-certs "$out/SamsungFeatureExt-v2.9.18.apk"

Copy-Item -Force "$out/SamsungFeatureExt-v2.9.18.apk" (Join-Path $workspaceDir 'SamsungWebDavRawMergedLsp.apk')
Copy-Item -Force "$out/SamsungFeatureExt-v2.9.18.apk" (Join-Path $workspaceDir 'MyFilesWebDavPopupLsp.apk')

$outputs = @(
    "$out/SamsungFeatureExt-v2.9.18.apk",
